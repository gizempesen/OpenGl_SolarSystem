...
class Planet{  //creating class
public:
	//creating constructor taking radius, distance, speed and tilts 
	...
	//creating class functions. One for smaller orbtial trails, one for the moons.
	void drawSmallOrbit(void){
		glPushMatrix();
		glColor3ub(255, 255, 255);        //orbit trails are white,
		glRotatef(90.0, 1.0, 0.0, 0.0);   //rotated so they are 'long ways' 
		glutWireTorus(0.001, distance, 100.0, 100.0);  //drawn as a torus with a small inner radius
		glPopMatrix();                                  // and the outher radius based on distance of the Moon from their planet
	}

	void drawMoon(void){
		GLUquadricObj *quadric;
		quadric = gluNewQuadric();  //Moons firstly take a quadric function for glutSphere
		glPushMatrix();
		glColor3ub(255, 255, 255);   //All moons are given white colouring.
		glRotatef(orbit, 0.0, 1.0, 0.0); //an orbital animation
		glTranslatef(distance, 0.0, 0.0);  //the distance from their planet for animation
...
//Calling a variety of planets with their neccesary data
...
int isAnimate = 0;
int bigOrbitActive = 1;  //toggling orbital trails
int smallOrbitActive = 1;
int moonsActive = 1;
int changeCamera = 0;  //changing camera angles
int frameCount = 0;  //counting frames
int labelsActive = 0;  //toggling labels
int zoom = 50; //zooming
int logoScene = 1;  //if the logo scene is active
...
GLuint loadTexture(Image* image) {
	//load texture function from 'codeinblocks' loads the bmps and returns texturelds
...
//Declaring the names of the different planet textures 
...
//Function that allows for text in the scene. Used for the planet labels.
...
void setup(void){
	...
	Image* sta = loadBMP("stars.bmp");		staTexture = loadTexture(sta);		delete sta;  //loading each of the textures from storage to the pre-declared variables
...
//Remainder of the animations function that checks when the orbit reaches 360 and then allows the animation to continue smoothly
void animate(int n){
	if (isAnimate){
		...
		//the scene will update every 30 seconds 
		mer.axisAni += 10.0;
		ven.axisAni += 10.0;
		ear.axisAni += 10.0;
		mar.axisAni += 10.0;
...
//Mouse click control that sets logoScene to be 0 when left button is down
void mouseControl(int button, int state, int x, int y)
{
	if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN)
		if (logoScene) logoScene = 0;

	if (button == GLUT_RIGHT_BUTTON && state == GLUT_DOWN) exit(0);  //when right button is clicked, the scene will close based off exit(0)
	glutPostRedisplay();
}
...
//simple function that just prints control/instruction to the console.
